"""
OASIS simulation runner: runs simulations in the background, logs each agent's actions, supports real-time status.
"""
...
class SimulationRunState:
    ...
    def add_action(self, action: AgentAction):
        """Append action to recent_actions and update counts."""
    ...
    def to_detail_dict(self) -> Dict[str, Any]:
        """State dict including recent_actions and rounds_count."""
        result = self.to_dict()
...
class SimulationRunner:
    """Runs OASIS simulations in a subprocess, parses action logs, exposes real-time state, supports pause/stop."""
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """Return run state from memory or load from file."""
    ...
    def start_simulation(
        ...
        """Start simulation subprocess; returns initial run state."""
    ...
    def _read_action_log(
        ...
        """Read action log from file; return new file position."""
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        """Return True if every enabled platform (by presence of actions.jsonl) has completed."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, state.simulation_id)
        ...
        if IS_WINDOWS:
            ...
            try:
                ...
                try:
                    ...
                    logger.warning(f"Process not responding, force kill: {simulation_id}")
                    subprocess.run(
    ...
    def get_all_actions(
        ...
        """Return full action history for all platforms (no pagination)."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        actions = []
        twitter_actions_log = os.path.join(sim_dir, "twitter", "actions.jsonl")
    ...
    def cleanup_all_simulations(cls):
        """Stop all running simulations (e.g. on server shutdown)."""
        ...
        for simulation_id, process in processes:
            try:
                ...
                logger.error(f"Failed to cleanup process: {simulation_id}, error={e}")
    ...
    def register_cleanup(cls):
        """Register cleanup on server shutdown; call from Flask app startup."""
        ...
        try:
            ...
            logger.warning("Not in main thread, cannot register signal handlers; using atexit only")
        _cleanup_registered = True
    
    @classmethod
    def get_running_simulations(cls) -> List[str]:
        """Return list of simulation IDs that are currently running."""
        running = []
    ...
    def check_env_alive(cls, simulation_id: str) -> bool:
        """Return True if the simulation env is alive and can accept Interview commands."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            return False
    ...
    def get_env_status_detail(cls, simulation_id: str) -> Dict[str, Any]:
        """Return env status detail: status, twitter_available, reddit_available, timestamp."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
    ...
    def interview_agent(
        ...
        """Interview a single agent; returns response dict."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation not found: {simulation_id}")
    ...
    def interview_all_agents(
        ...
        if not agent_configs:
            raise ValueError(f"Simulation config has no agents: {simulation_id}")
        interviews = []
    ...
    def close_simulation_env(
        ...
        """Send close-env command to simulation so it exits command-wait mode (does not stop the process)."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        if not os.path.exists(sim_dir):
            raise ValueError(f"Simulation not found: {simulation_id}")
        ipc_client = SimulationIPCClient(sim_dir)
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Close-env sent (response timed out; env may be shutting down)"
            }
    ...
    def _get_interview_history_from_db(
        ...
        """Read interview history from a single DB file."""
        import sqlite3
    ...
    def get_interview_history(
        ...
        """Get interview history from DB (by platform and optional agent_id)."""
        sim_dir = os.path.join(cls.RUN_STATE_DIR, simulation_id)
        results = []
