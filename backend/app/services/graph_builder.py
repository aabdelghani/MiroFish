...
class GraphBuilderService:
    ...
    """Build knowledge graph via Zep API."""
    ...
    def _build_graph_worker(
        ...
        try:
            ...
            self.task_manager.update_task(
                task_id,
                progress=60,
                message="Waiting for Zep to process..."
            )
    ...
    def add_text_batches(
        ...
        """Add text in batches; return list of episode UUIDs."""
        episode_uuids = []
        total_chunks = len(chunks)
    ...
    def _wait_for_episodes(
        ...
        if not episode_uuids:
            if progress_callback:
                progress_callback("Nothing to wait for", 1.0)
            return
        ...
        if progress_callback:
            progress_callback(f"Waiting for {total_episodes} chunks...", 0)
        
        while pending_episodes:
            if time.time() - start_time > timeout:
                if progress_callback:
                    progress_callback(
                        f"Timeout: {completed_count}/{total_episodes} done",
            ...
            if progress_callback:
                progress_callback(
                    f"Zep: {completed_count}/{total_episodes} done, {len(pending_episodes)} pending ({elapsed}s)",
    ...
    def _get_graph_info(self, graph_id: str) -> GraphInfo:
        ...
        # 统计实体类型
        """Get graph info (nodes, edges, entity types)."""
    ...
    def get_graph_data(self, graph_id: str) -> Dict[str, Any]:
        ...
        """Get full graph data (nodes, edges, temporal and attribute details)."""
        nodes = fetch_all_nodes(self.client, graph_id)
        edges = fetch_all_edges(self.client, graph_id)
        node_map = {}
        for node in nodes:
